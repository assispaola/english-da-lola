...
- **Escopo por entidade**:
  ...
  - *Cross-level* (sem campo `level`): streak de dias estudando (estudar em qualquer nível conta), metas semanais, sessões de prep VIP, aulas agendadas (`classes`).
...
- **Autoavaliação de "Fala"**: não existe reconhecimento de voz nem qualquer verificação automática de pronúncia no app. Marcar um tópico da categoria Fala como "Concluído" (`Roadmap.jsx`, `cycleStatus`) abre um modal obrigatório (`SpeakingAssessmentModal.jsx`) pedindo uma nota honesta (fácil/médio/difícil + observação opcional) antes de aplicar a mudança de status — fechar sem responder não altera nada. Cada resposta vira um registro em `src/utils/speakingAssessments.js` (independente do status do roadmap, histórico completo por tópico), e só depois disso `completedAt` é gravado — o % de "Fala" no dashboard usa a mesma fórmula das outras categorias, só que agora tem uma autoavaliação por trás em vez de um clique simples. O ícone (ⓘ) ao lado do label "Fala" no card "progresso por skill" deixa isso explícito para quem estiver lendo o gráfico.
...
- `src/utils/storage.js` — `createStore(key, syncName)`: o segundo argumento é o nome da coleção no Firestore. Entidades geradas pelo usuário (flashcards, notas, erros, tentativas, diário, glossário, aulas) passam `syncName`; conteúdo estático re-semeado localmente em todo dispositivo (tópicos, exercícios) **não** passa — nunca precisa tocar a nuvem.
- `src/utils/classes.js` — CRUD de aulas agendadas (`{date, topic, type: 'Grupo'|'VIP'}`, sem `level`), usado pelo card "próximas aulas" do Dashboard (`ClassesCard.jsx`). Migra automaticamente (uma única vez, `migrateLegacyNextClass()`) o antigo valor único de "next class" para a lista nova.
- `src/utils/speakingAssessments.js` — CRUD do histórico de autoavaliações de fala (`{topicId, rating, note, level, timestamp}`), ver "Autoavaliação de Fala" acima.
- Estrutura no Firestore: `users/{uid}` (perfil: `current_level`, `levels_unlocked`, `migratedAt`), `users/{uid}/{coleção}/{id}` (flashcards, diario, glossario, notes, errorLog, attempts, classes, speakingAssessments), `users/{uid}/settings/{chave}` (vipCurrent, vipSessions, metas), `users/{uid}/roadmapProgress/{level}`.
