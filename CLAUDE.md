...
- **Firebase Auth** (login com Google) **+ Firestore** para persistência multiusuário-por-nível — ver "Nuvem e sincronização" abaixo.
...
- `src/utils/auth.js` — `signInWithGoogle()`, `signOutUser()`, `onAuthChange()`.
...
- Regras de segurança: `firestore.rules` na raiz — cada usuária só lê/escreve o próprio `users/{uid}/**`. Colar no console do Firebase em **Firestore Database → Regras**.
...
2. No console do Firebase: ativar **Authentication → método de login Google**, adicionar o domínio de deploy em **Authentication → Settings → Authorized domains**, criar o **Firestore Database**, e colar o conteúdo de `firestore.rules` em **Firestore Database → Regras**.
