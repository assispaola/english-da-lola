    ...
    <div className="max-w-3xl">
      ...
      {todayPhrase ? (
        <div className="relative overflow-hidden rounded-2xl p-7 mb-5 text-white shadow-lg"
          ...
          <div className="relative z-10">
            <p className="font-body text-xs uppercase tracking-widest mb-3" style={{ color: 'rgba(255,255,255,0.75)' }}>💬 frase do dia</p>
            ...
            <div className="flex items-center gap-3 mt-5 flex-wrap">
              <span className="pill text-xs px-3" style={{ backgroundColor: 'rgba(255,255,255,0.20)', color: 'white' }}>
                {todayPhrase.category}
              </span>
              <button onClick={() => markMastered(todayPhrase.id)}
                className="font-body font-semibold text-sm px-4 py-2 transition-all hover:opacity-90"
                style={{ backgroundColor: 'rgba(255,255,255,0.20)', color: 'white', border: '1.5px solid rgba(255,255,255,0.40)', minHeight: '40px', borderRadius: '8px' }}>
                ✅ já sei de cor!
              </button>
            </div>
          </div>
        ...
        <div className="rounded-2xl p-8 mb-5 text-center border-2 border-dashed" style={{ backgroundColor: pc.accent, borderColor: pc.border }}>
          <div className="text-5xl mb-3">💬</div>
          <p className="font-heading text-xl lowercase" style={{ color: pc.primary, fontWeight: 500 }}>add phrases to start practicing!</p>
        </div>
      ) : (
      ...
      {showForm && (
        <div className="card p-5 mb-5" style={{ borderColor: pc.border }}>
          <h3 className="font-heading text-lg mb-4 lowercase" style={{ color: pc.primary, fontWeight: 500 }}>
      ...
      <div className="space-y-3">
        {phrases.map((phrase, i) => {
          ...
          return (
            <div key={phrase.id}
              className={`p-4 border-[1.5px] transition-all hover:scale-[1.01] group ${phrase.mastered ? 'opacity-60' : ''}`}
              style={{
                backgroundColor: phrase.mastered ? '#F9FAFB' : cs.accent,
                borderColor: phrase.mastered ? '#E5E7EB' : cs.border,
                borderRadius: '16px',
                boxShadow: phrase.mastered ? 'none' : '0 2px 8px rgba(0,0,0,0.06)',
              }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {todayPhrase?.id === phrase.id && !phrase.mastered && (
                      <span className="pill text-xs" style={{ backgroundColor: cs.border, color: cs.text }}>📅 hoje</span>
                    )}
                  </div>
                  <p className="font-heading text-base lowercase" style={{ color: cs.primary, fontWeight: 500 }}>{phrase.english}</p>
